...
$client  = new OAuth2\Client(CLIENT_ID, CLIENT_SECRET);
 ...
 $app->get('/', function() use ($app, $client) {
     return check_auth($app, $client);
 });

 $app->match('/pledge', function (Request $request) use ($app, $client) {
     // Get and set token.
     $token = set_token($app, $client);

     // Fetch the user's profile.
     $response = $client->fetch(REQUEST_ENDPOINT . '/people/me.json');
     $id = $response['result']['person']['id'];
...
/**
 * FIXME Submit the data to NationBuilder.
 */
$app->post('/pledge/submit', function (Request $request) use ($app, $client, $data, $token) {
    // set_token($app, $client);
    // global $data, $token;

    // TODO Submit the data to NationBuilder.
    // Set the headers for the update request.
    $header = array(
    	'Authorization' => $token,
    ...
    $response = $client->fetch(REQUEST_ENDPOINT . "/people/" . $id, $token, $params, "POST", $header);
